...
PKG_PATH=${ICP_ROOT}/Package_RD_Agent_V2R1C20/code/current/AGENT_PACK_TEMP 
...
!
        ...
        then
           ...
           umount ${MOUNT_PATH}
           mount -t cifs -o username=Administrator,password=Huawei@123 //10.183.243.153/RDV2R1C20_AgentPackage ${MOUNT_PATH}
           if [ $? -ne 0 ]
           then
             echo "ERROR:mount //10.183.243.153/RDV2R1C20_AgentPackage to ${MOUNT_PATH} failed!"
             exit 1
           fi
