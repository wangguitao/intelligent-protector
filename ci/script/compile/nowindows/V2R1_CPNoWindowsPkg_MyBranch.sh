...
PKG_PATH=${ICP_ROOT}/Package_RD_Agent_MyBranch/code/current/AGENT_PACK_TEMP
...
!
        ...
        MOUNT_PATH=/mnt
		MOUNT_PATH_OTHER=/mnt_mybranch

        if [ "${ISSUSE}" = "1" ] && [ "${VERSION}" = "12" ]
        then
           PKG_NAME=`ls ${PKG_PATH} | grep "x86_64.tar.gz"`
           cp "${PKG_PATH}/${PKG_NAME}" "${MOUNT_PATH_OTHER}/pkg/"
        else
           umount ${MOUNT_PATH}
           mount -t cifs -o username=Administrator,password=Huawei@123 //10.183.243.153/MyBranch_AgentPackage ${MOUNT_PATH}
           if [ $? -ne 0 ]
           then
             echo "ERROR:mount //10.183.243.153/MyBranch_AgentPackage to ${MOUNT_PATH} failed!"
             exit 1
           fi
